...
(function () {
  ...
  const requestTemp = () => {
    ...
    request.onload = function () {
      if (request.readyState == 4 && request.status == 200) {
        setGauge(request.responseText); // set gauge value
      }
    };
  ...
  document.querySelector(".gauge-container").addEventListener("click", () => {
    gaugeMode === 2 ? setGaugeMode(1) : setGaugeMode(2);
  });

  const setGauge = (num, mode = 2) => {
    setGaugeMode(mode);
    setGaugeThemperatureNumber(num);
    setGaugeArrowAngle(num);
    setGaugeArcLine(num);
  };
